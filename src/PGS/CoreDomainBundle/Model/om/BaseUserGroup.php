...
abstract class BaseUserGroup extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'PGS\\CoreDomainBundle\\Model\\UserGroupPeer';
    ...
    /**
     * The value for the fos_group_id field.
     * @var        int
     */
    ...
    /**
     * Get the [fos_group_id] column value.
     *
     * @return int
     */
    ...
    public function setFosUserId($v)
    {
        ...
        if ($this->fos_user_id !== $v) {
            $this->fos_user_id = $v;
            $this->modifiedColumns[] = UserGroupPeer::FOS_USER_ID;
        }
    ...
    /**
     * Set the value of [fos_group_id] column.
     *
     * @param  int $v new value
     * @return UserGroup The current object (for fluent API support)
     */
    public function setFosGroupId($v)
    {
        if ($v !== null && is_numeric($v)) {
            $v = (int) $v;
        }

        if ($this->fos_group_id !== $v) {
            $this->fos_group_id = $v;
            $this->modifiedColumns[] = UserGroupPeer::FOS_GROUP_ID;
        }
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aGroup !== null && $this->fos_group_id !== $this->aGroup->getId()) {
            $this->aGroup = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UserGroupPeer::FOS_USER_ID)) {
    ...
    {
        $criteria = new Criteria(UserGroupPeer::DATABASE_NAME);
        $criteria->add(UserGroupPeer::FOS_USER_ID, $this->fos_user_id);
        $criteria->add(UserGroupPeer::FOS_GROUP_ID, $this->fos_group_id);
    ...
    {
        $pks = array();
        $pks[0] = $this->getFosUserId();
        $pks[1] = $this->getFosGroupId();
    ...
    {
        $this->setFosUserId($keys[0]);
        $this->setFosGroupId($keys[1]);
    }
    ...
    public function isPrimaryKeyNull()
    {
        ...
        return (null === $this->getFosUserId()) && (null === $this->getFosGroupId());
    }
    ...
    {
        if ($this->aGroup === null && ($this->fos_group_id !== null) && $doQuery) {
            $this->aGroup = GroupQuery::create()->findPk($this->fos_group_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
