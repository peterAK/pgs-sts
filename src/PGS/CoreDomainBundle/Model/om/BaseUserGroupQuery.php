...
abstract class BaseUserGroupQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'PGS\\CoreDomainBundle\\Model\\UserGroup';
        }
    ...
    {
        $sql = 'SELECT `fos_user_id`, `fos_group_id`, `created_at`, `updated_at` FROM `fos_user_group` WHERE `fos_user_id` = :p0 AND `fos_group_id` = :p1';
        try {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \PGS\CoreDomainBundle\Model\GroupQuery A secondary query class using the current class as primary query
     */
    public function useGroupQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Group', '\PGS\CoreDomainBundle\Model\GroupQuery');
    }
